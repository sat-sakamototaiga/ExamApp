    ...
    <div class="max-w-6xl mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        <div class="mb-6 grid gap-6 lg:grid-cols-2">
            <section class="rounded-lg border border-gray-200 p-5">
                <details {{ $openIndividualForm ? 'open' : '' }}>
                    <summary class="cursor-pointer list-none">
                        <h2 class="inline text-lg font-bold text-gray-900">教師・生徒を個別登録</h2>
                        <span class="ms-2 text-sm text-blue-700">クリックで開閉</span>
                        <p class="mt-1 text-sm text-gray-600">管理者が教師または生徒アカウントを直接追加できます。</p>
                    </summary>

                    <form action="{{ route('admin.users.store') }}" method="POST" class="mt-4 space-y-4 border-t border-gray-100 pt-4">
                        @csrf
            ...
            <section class="rounded-lg border border-gray-200 p-5">
                <details {{ $openCsvForm ? 'open' : '' }}>
                    <summary class="cursor-pointer list-none">
                        <h2 class="inline text-lg font-bold text-gray-900">CSVで一括登録</h2>
                        <span class="ms-2 text-sm text-blue-700">クリックで開閉</span>
                    ...
                    <form action="{{ route('admin.users.import') }}" method="POST" enctype="multipart/form-data" class="mt-4 space-y-4 border-t border-gray-100 pt-4">
                        @csrf
